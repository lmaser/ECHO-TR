...
- **WIDE**: Cross-feedback between channels with pitch-compensated octave offset. The delays are scaled so that the cross-feedback round trip (`T_L + T_R`) equals the user's delay time `T`, keeping the comb-filter resonance at the same pitch as STEREO mode. The ratio between channels is 2:1 (`L = 2T/3`, `R = T/3`), preserving the octave character that spreads repetitions across the stereo field. Unlike DUAL (which uses a `x0.5` ratio with independent feedback), WIDE uses cross-feedback - each channel feeds into the other - creating a widening bounce pattern at the correct pitch.
- **DUAL**: Independent left/right delay lines with the right channel at half the delay time (`x0.5`). No cross-feedback - each channel repeats independently at different rates.
...
Automatic pitch-scaling: shorter delays recover faster (`sqrt(delay / 1000)`).
...
- Switching between modes does not corrupt the buffer.
...
- `-2 = x0.25` (very short taper -> choppy, granular)
- `0 = x1.0` (clean default)
- `+2 = x4.0` (long taper -> ambient, washy)
...
- Fast dB-to-linear conversion via `std::exp2`.
- Gain and mix smoothing snap to target when within epsilon to avoid useless work in steady state.
