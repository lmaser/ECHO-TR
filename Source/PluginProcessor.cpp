...
namespace
{
	...
	{
		return (dB <= ECHOTRAudioProcessor::kGainFloorDb) ? 0.0f : std::exp2 (dB * 0.16609640474f);
	}

	inline juce::NormalisableRange<float> makeGainFaderRange() noexcept
	{
		return juce::NormalisableRange<float> (ECHOTRAudioProcessor::kGainFloorDb,
		                                       ECHOTRAudioProcessor::kGainMaxDb,
		                                       0.0f,
		                                       ECHOTRAudioProcessor::kGainSkew);
	}
	...
	static_assert (kNumTimeSyncDivisions == ECHOTRAudioProcessor::kTimeSyncMax + 1,
...
void ECHOTRAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
	...
	duckAmount_ = 0.0f;
	{
		...
		duckAttackCoeff_  = static_cast<float> (1.0f - std::exp (-1.0f / (sr * 0.001f)));  // ~1 ms
		const auto makeDuckReleaseCoeff = [] (float releaseMs, float sampleRate) noexcept
		{
			return static_cast<float> (1.0 - std::exp (-1.0 / (sampleRate * (releaseMs * 0.001f))));
		};
		duckReleaseCoeffMin_ = makeDuckReleaseCoeff (120.0f, sr);
		duckReleaseCoeffMax_ = makeDuckReleaseCoeff (200.0f, sr);
	}
                                                ...
                                                float outputGain, float /*mix*/, float delaySmoothCoeff)
{
	...
	for (int i = 0; i < numSamples; ++i)
	{
		...
		float readPosF = (float) writePos - readDelaySamples;
		if (readPosF < 0.0f)
                                              ...
                                              float outputGain, float /*mix*/, float delaySmoothCoeff)
{
	...
	for (int i = 0; i < numSamples; ++i)
	{
		...
		float readPosL = (float) writePos - readDelayL;
		if (readPosL < 0.0f)
		...
		float readPosR = (float) writePos - readDelayR;
		if (readPosR < 0.0f)
                                                 ...
                                                 float smoothMult, int mode)
{
	...
	for (int i = 0; i < numSamples; ++i)
	{
		...
		if (reverseNeedsInit)
		{
			...
			reverseCounter   = 0.0f;
			const float candidateLen = juce::jmax (kMinChunkLen, jitteredReverseDelay);
		...
		const float fwdDelayL = juce::jmax (2.0f, jitteredReverseDelay * fbkRatioL);
		...
		const float fwdDelayR = juce::jmax (2.0f, jitteredReverseDelay * fbkRatioR);
		...
		if (reverseCounter >= chunkLen)
		{
			reverseCounter  = 0.0f;
			reverseAnchor   = writePos;
			const float newLen = juce::jmax (kMinChunkLen, jitteredReverseDelay);
...
void ECHOTRAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
	...
	if (numChannels >= 2 && modeOutVal != 0)
	{
		...
		for (int i = 0; i < numSamples; ++i)
		{
			const float mono = (chL[i] + chR[i]) * 0.5f;
			if (modeOutVal == 1)
			{
				chL[i] = mono;
				chR[i] = mono;
	...
	if (numChannels >= 2 && sumBusVal != 0)
	{
		...
		for (int i = 0; i < numSamples; ++i)
		{
			...
			if (modeOutVal != 0)
			{
				const float mono = (encL + encR) * 0.5f;
				if (modeOutVal == 1)
				{
					encL = mono;
					encR = mono;
